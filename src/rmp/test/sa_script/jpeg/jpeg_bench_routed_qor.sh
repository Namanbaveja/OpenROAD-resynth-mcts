...
# post-PnR jpeg_final design and aggregate baseline-vs-after WNS/TNS/FEPs QoR.
#
# Live output:
...
#
# Requirements: openroad in PATH, run from sa_script/jpeg/.
#   ./jpeg_bench_routed_qor.sh
#   SEEDS="50 100" PERCENTAGES="10 20 30" ITERS="100 200" ./jpeg_bench_routed_qor.sh
#   OPENROAD=../../../build/bin/openroad ./jpeg_bench_routed_qor.sh
#   DEBUG_LEVEL=0 ./jpeg_bench_routed_qor.sh   # suppress per-iter debug noise
#   EXPORT_FILES=1 ./jpeg_bench_routed_qor.sh  # also write DEF/v/sdc/spef
#   NO_COLOR=1 ./jpeg_bench_routed_qor.sh      # disable ANSI colors
set -u
...
TCL="${TCL:-jpeg_routed_qor.tcl}"

# Derive design name from the TCL filename (jpeg_routed_qor.tcl → "jpeg").
...
if [[ ! -f "${TCL}" ]]; then
  echo "ERROR: ${TCL} not found; run from sa_script/jpeg/" >&2
  exit 1
fi
