#!/usr/bin/env bash
# Sweep resynth_mcts on post-CTS usb_phy (ASAP7).
# Run from mcts_script/usb_phy/:
#   ./usb_phy_bench_routed_qor_mcts.sh
#   SEEDS="50 100" PERCENTAGES="10 20 30" ITERS="500" ./usb_phy_bench_routed_qor_mcts.sh
#   OPENROAD=../../../../build/bin/openroad ./usb_phy_bench_routed_qor_mcts.sh
set -u
...
TCL="${TCL:-usb_phy_routed_qor_mcts.tcl}"
...
if [[ ! -f "${TCL}" ]]; then
  echo "ERROR: ${TCL} not found; run from mcts_script/usb_phy/" >&2
  exit 1
fi
