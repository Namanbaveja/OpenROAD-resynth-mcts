# test for set_voltage_domain -region with a different name
source "helpers.tcl"
...
add_pdn_ring -grid "Core" -layers {met4 met5} -widths 5.0 -spacings 2.0 -core_offsets 4.0

add_pdn_connect -grid "Core" -layers {met1 met4}
add_pdn_connect -grid "Core" -layers {met4 met5}

set_voltage_domain -power VIN -ground VSS -region "TEMP_ANALOG" -name "TemperatureAnalog"
define_pdn_grid -name "TempSensor" -voltage_domains "TemperatureAnalog"
add_pdn_stripe -followpins -layer met1 -width 0.48 -extend_to_core_ring
