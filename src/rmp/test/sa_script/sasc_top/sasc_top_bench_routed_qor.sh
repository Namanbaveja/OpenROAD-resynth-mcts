...
# post-PnR sasc_top_final design and aggregate baseline-vs-after WNS/TNS/FEPs QoR.
#
# Live output:
...
#
# Requirements: openroad in PATH, run from sa_script/sasc_top/.
#   ./sasc_top_bench_routed_qor.sh
#   SEEDS="50 100" PERCENTAGES="10 20 30" ITERS="100 200" ./sasc_top_bench_routed_qor.sh
#   OPENROAD=../../../build/bin/openroad ./sasc_top_bench_routed_qor.sh
#   DEBUG_LEVEL=0 ./sasc_top_bench_routed_qor.sh   # suppress per-iter debug noise
#   EXPORT_FILES=1 ./sasc_top_bench_routed_qor.sh  # also write DEF/v/sdc/spef
#   NO_COLOR=1 ./sasc_top_bench_routed_qor.sh      # disable ANSI colors
set -u
...
TCL="${TCL:-sasc_top_routed_qor.tcl}"

# Derive design name from the TCL filename (sasc_top_routed_qor.tcl → "sasc_top").
...
if [[ ! -f "${TCL}" ]]; then
  echo "ERROR: ${TCL} not found; run from sa_script/sasc_top/" >&2
  exit 1
fi
