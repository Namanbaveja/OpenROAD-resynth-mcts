...
# post-CTS aes design (ASAP7) and aggregate baseline-vs-after WNS/TNS/FEP QoR.
#
# Run from this directory (mcts_script/aes/):
#   ./aes_bench_routed_qor_mcts.sh
#   SEEDS="50 100" PERCENTAGES="10 20 30" ITERS="100 200" ./aes_bench_routed_qor_mcts.sh
#   OPENROAD=../../../../build/bin/openroad ./aes_bench_routed_qor_mcts.sh
#   DEBUG_LEVEL=0 ./aes_bench_routed_qor_mcts.sh
#   EXPORT_FILES=1 ./aes_bench_routed_qor_mcts.sh
#   NO_COLOR=1 ./aes_bench_routed_qor_mcts.sh
set -u
...
TCL="${TCL:-aes_routed_qor_mcts.tcl}"

DESIGN="${DESIGN:-aes}"

# Logs land in ../../qor_mcts/aes/ (relative to this script's directory)
...
if [[ ! -f "${TCL}" ]]; then
  echo "ERROR: ${TCL} not found; run from mcts_script/aes/" >&2
  exit 1
fi
