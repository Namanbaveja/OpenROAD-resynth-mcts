...
# post-CTS ac97 design (ASAP7) and aggregate baseline-vs-after WNS/TNS/FEP QoR.
#
# Run from this directory (mcts_script/ac97/):
#   ./ac97_bench_routed_qor_mcts.sh
#   SEEDS="50 100" PERCENTAGES="10 20 30" ITERS="100 200" ./ac97_bench_routed_qor_mcts.sh
#   OPENROAD=../../../../build/bin/openroad ./ac97_bench_routed_qor_mcts.sh
#   DEBUG_LEVEL=0 ./ac97_bench_routed_qor_mcts.sh
#   EXPORT_FILES=1 ./ac97_bench_routed_qor_mcts.sh
#   NO_COLOR=1 ./ac97_bench_routed_qor_mcts.sh
set -u
...
TCL="${TCL:-ac97_routed_qor_mcts.tcl}"

DESIGN="${DESIGN:-ac97}"

# Logs land in ../../qor_mcts/ac97/ (relative to this script's directory)
...
if [[ ! -f "${TCL}" ]]; then
  echo "ERROR: ${TCL} not found; run from mcts_script/ac97/" >&2
  exit 1
fi
