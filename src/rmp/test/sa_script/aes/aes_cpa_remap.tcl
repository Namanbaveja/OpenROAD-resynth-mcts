...
# CPA-remap test on AES post-CTS benchmark (ASAP7)
# Usage: openroad -no_init aes_cpa_remap.tcl
#
# Env-var overrides (all optional):
#   CORNER             slow|fast   (default slow)
#   BFS_HOPS           K-hop BFS expansion depth (default 1)
#   MAX_INST           max instances per cone (default 200)
...
read_def ../../benchmark_cts/cts_database/asap7/aes/base/4_cts.def
read_sdc ../../benchmark_cts/cts_database/asap7/aes/base/4_cts.sdc
...
# ---- baseline ----
lassign [snapshot_qor "BASELINE (4_cts.def)"] wns_before tns_before viols_before

remove_fillers

puts ""
puts ">>> Config: corner=$corner bfs_hops=$bfs_hops max_inst=$max_inst proximity_um=$prox_um long_wire=${long_wire}ps mcts_iters=$mcts_iters max_cones=$max_cones"
puts ""
...
if { $prox_um != 0.0 }  { append cmd " -proximity_um $prox_um" }
if { $long_wire != 0 }  { append cmd " -long_wire_ps $long_wire" }
if { $mcts_iters > 0 }  { append cmd " -mcts_iters $mcts_iters -mcts_max_depth $mcts_max_depth -mcts_ucb_constant $mcts_ucb" }
if { $max_cones > 0 }   { append cmd " -max_cones_per_round $max_cones" }
...
puts [format "  FEPs:%12d     ->  %12d    delta = %+d" \
          $viols_before $viols_final $viols_delta]
puts [format "  TIME: cpa=%dms  repair=%dms" $elapsed_cpa $elapsed_repair]
puts ""
puts [format "RESULT design=aes corner=%s bfs=%s max_inst=%s prox_um=%.1f long_wire_ps=%s mcts=%s max_cones=%s with_repair=%s wns_before=%.4f wns_cpa=%.4f wns_final=%.4f tns_before=%.4f tns_cpa=%.4f tns_final=%.4f viols_before=%d viols_cpa=%d viols_final=%d cpa_ms=%d repair_ms=%d" \
