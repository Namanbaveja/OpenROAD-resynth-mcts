...
proc write_hi_fanout_def1 {
  ...
  write_fanout_loads $stream $fanout $load_inst $load_cell $load_in \
    $load_space
...
proc write_fanout_loads { stream fanout load_inst load_cell load_in load_space } {
  ...
  set i 0
  while { $i < $fanout } {
    puts $stream "- $load_inst$i $load_cell + PLACED ( \
                    [expr { ($i % $row_count) * $load_space }]\
                    [expr { ($i / $row_count) * $load_space }] ) N ;"
    incr i
  }
}
...
# drvr_inst/drvr_out -> load0/load_in rload1/load_in .... load<fanout>/load_in
proc write_clone_test_def1 {
  filename clone_gate fanout
  ...
  # We have the three input nets, then we have the two nets that connect
  # the DFF output to the gate input
  ...
  puts $stream "- clk_to_nand0 ( ${drvr_inst}_1 $drvr_out )\
                  ( nand_inst_0 A1 ) ;"
  puts $stream "- clk_to_nand1 ( ${drvr_inst}_2 $drvr_out )\
                  ( nand_inst_0 A2 ) ;"
