...
# post-CTS jpeg design (ASAP7) and aggregate baseline-vs-after WNS/TNS/FEP QoR.
#
# Run from this directory (mcts_script/jpeg/):
#   ./jpeg_bench_routed_qor_mcts.sh
#   SEEDS="50 100" PERCENTAGES="10 20 30" ITERS="100 200" ./jpeg_bench_routed_qor_mcts.sh
#   OPENROAD=../../../../build/bin/openroad ./jpeg_bench_routed_qor_mcts.sh
#   DEBUG_LEVEL=0 ./jpeg_bench_routed_qor_mcts.sh
#   EXPORT_FILES=1 ./jpeg_bench_routed_qor_mcts.sh
#   NO_COLOR=1 ./jpeg_bench_routed_qor_mcts.sh
set -u
...
TCL="${TCL:-jpeg_routed_qor_mcts.tcl}"

DESIGN="${DESIGN:-jpeg}"

# Logs land in ../../qor_mcts/jpeg/ (relative to this script's directory)
...
if [[ ! -f "${TCL}" ]]; then
  echo "ERROR: ${TCL} not found; run from mcts_script/jpeg/" >&2
  exit 1
fi
