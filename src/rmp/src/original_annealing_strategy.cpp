...
void OriginalAnnealingStrategy::OptimizeDesign(sta::dbSta* sta,
                                       ...
                                       utl::Logger* logger)
{
  ...
  sta->ensureLevelized();
  sta->searchPreamble();
  for (auto mode : sta->modes()) { sta->ensureClkNetwork(mode); }
  ...
  // Greedy top-k% cone selection: sort by slack ascending (most violated
  // first), keep the worst percentage_% endpoints — same filter as resynth_sa.
    ...
    std::partial_sort(
        ...
        [sta](sta::Vertex* a, sta::Vertex* b) {
          return sta->slack(a, sta::MinMax::max())
               < sta->slack(b, sta::MinMax::max());
        });
    candidate_vertices.resize(keep);
    logger->info(utl::RMP, 510,
                 "resynth_annealing: top-{:.0f}% selection: {}/{} endpoints",
                 static_cast<double>(percentage_), keep, total);
  }
  ...
  std::vector<OriginalGiaOp> all_ops = {
      [&](auto& gia) {
        // &st
  ...
  const auto neighbor = [&](std::vector<OriginalGiaOp> ops) {
    enum Move
    {
      ADD,
      REMOVE,
  ...
  // Use pure TNS (total negative slack) as the score — same objective as
  // resynth_sa.  Higher (less negative) = better.
  // RunGia already calls delaysInvalid/arrivalsInvalid so STA re-propagates
  // fresh on the first totalNegativeSlack call.
  sta::Slack tns = sta->totalNegativeSlack(corner_, sta::MinMax::max());

  odb::dbDatabase::undoEco(block);

  if (!temperature_) {
    // T₀ = |initial TNS| so first-move acceptance ≈ 1/e (same calibration
    // as resynth_sa).
    constexpr float kMinT0 = 1.0e-13f;
    temperature_ = std::max(static_cast<float>(std::abs(static_cast<double>(tns))), kMinT0);
  }

  logger->info(RMP, 500, "Resynthesis: starting simulated annealing (TNS oracle)");
  logger->info(RMP,
               53,
               "Initial temperature: {}, initial TNS: {}",
               *temperature_,
               tns);

  sta::Slack best_tns = tns;
  auto best_ops = ops;
  ...
  for (unsigned i = 0; i < iterations_; i++) {
    ...
    if (i > 0 && (i + 1) % 10 == 0) {
      logger->info(RMP,
                   54,
                   "Iteration: {}, temperature: {}, best TNS: {}",
                   i + 1,
                   current_temp,
                   best_tns);
    } else {
    ...
    sta::Slack tns_new = sta->totalNegativeSlack(corner_, sta::MinMax::max());

    odb::dbDatabase::undoEco(block);

    // tns_new < tns means new solution is worse (more negative total slack).
    if (tns_new < tns) {
      float accept_prob
          = current_temp == 0
                ? 0
                : std::exp(static_cast<float>(tns_new - tns) / current_temp);
      debugPrint(logger, RMP, "annealing", 1,
                 "TNS: current={}, new={}, accept_prob={}",
                 tns, tns_new, accept_prob);
      if (std::uniform_real_distribution<float>(0, 1)(random_) >= accept_prob) {
        debugPrint(logger, RMP, "annealing", 1, "Rejecting worse TNS");
        continue;
      }
      debugPrint(logger, RMP, "annealing", 1, "Accepting worse TNS");
    } else {
      debugPrint(logger, RMP, "annealing", 1,
                 "TNS: current={}, new={}, accepting (improved)",
                 tns, tns_new);
    }
  ...
  logger->info(
      RMP, 501, "Resynthesis: End of simulated annealing, applying operations");
  logger->info(RMP, 504, "Resynthesis: Applying ABC operations");

  // Apply the ops — stitches restructured cone back into ODB netlist.
  // New cells are left UNPLACED; caller (TCL bench script) handles
  // detailed_placement / estimate_parasitics / repair_timing afterwards.
  RunGia(sta,
         candidate_vertices,
         abc_library,
         best_ops,
...
void OriginalAnnealingStrategy::RunGia(
    sta::dbSta* sta,
