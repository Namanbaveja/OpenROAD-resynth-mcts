#!/usr/bin/env bash
# Sweep resynth_annealing on post-CTS simple_spi_top (ASAP7).
# Run from sa_script/simple_spi_top/:
#   ./simple_spi_top_bench_routed_qor.sh
#   SEEDS="50 100" PERCENTAGES="10 20 30" ITERS="500" ./simple_spi_top_bench_routed_qor.sh
#   OPENROAD=../../../../build/bin/openroad ./simple_spi_top_bench_routed_qor.sh
set -u
...
TCL="${TCL:-simple_spi_top_routed_qor.tcl}"

DESIGN="${DESIGN:-simple_spi_top}"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TEST_DIR="${SCRIPT_DIR}/../.."
LOG_ROOT="${LOG_ROOT:-${TEST_DIR}/qor_sa}"
LOG_DIR="${LOG_DIR:-${LOG_ROOT}/${DESIGN}}"

if [[ ! -f "${TCL}" ]]; then
  echo "ERROR: ${TCL} not found; run from sa_script/simple_spi_top/" >&2
  exit 1
fi
...
if [[ -t 1 && -z "${NO_COLOR:-}" ]]; then
  ...
  C_RESET=''; C_BOLD=''; C_DIM=''; C_RED=''; C_GREEN=''; C_YELLOW=''; C_CYAN=''; CLR_LINE=$'\r'
fi

timestamp="$(date +%Y%m%d_%H%M%S)"
results_tsv="${LOG_DIR}/${DESIGN}_sa_results_${timestamp}.tsv"
results_txt="${LOG_DIR}/${DESIGN}_sa_results_${timestamp}.txt"
results_file="$(mktemp)"
...
render_progress() {
  ...
  local bar_width=30
  awk -v logfile="${logfile}" -v total="${total_iters}" -v width="${bar_width}" \
      -v header="${header}" -v cyan="${C_CYAN}" -v dim="${C_DIM}" -v bold="${C_BOLD}" \
      -v reset="${C_RESET}" -v clr="${CLR_LINE}" '
    BEGIN { last_ms = 0; cur = 0; best = ""; draw(0, "") }
    function now_ms(   t) { "date +%s%3N" | getline t; close("date +%s%3N"); return t + 0 }
    function draw(n, wns,   filled, pct, i, bar) {
      ...
      printf "%s%s %s[%s%s%s] %d/%d (%.0f%%)  best WNS=%s%s%s",
             clr, header, dim, reset, bar, dim, n, total, pct, bold, wns, reset > "/dev/stderr"
      fflush("/dev/stderr")
    }
    { print $0 >> logfile
      if (match($0, /Iter[ \t]+([0-9]+)\/([0-9]+).*best WNS=([^ ]+)/, m)) {
        cur = m[1] + 0; best = m[3]; nowms = now_ms()
        if (nowms - last_ms >= 100 || cur == total) { draw(cur, best); last_ms = nowms }
      }
    }
...
fmt_delta() {
  ...
  case "${mode}" in
    wns|tns) if awk -v v="${val}" 'BEGIN{exit !(v+0 > 0)}'; then color="${C_GREEN}"
             elif awk -v v="${val}" 'BEGIN{exit !(v+0 < 0)}'; then color="${C_RED}"
             else color="${C_DIM}"; fi ;;
    fep)     if awk -v v="${val}" 'BEGIN{exit !(v+0 < 0)}'; then color="${C_GREEN}"
             elif awk -v v="${val}" 'BEGIN{exit !(v+0 > 0)}'; then color="${C_RED}"
             else color="${C_DIM}"; fi ;;
  esac
...
for seed in ${SEEDS}; do
  for pct in ${PERCENTAGES}; do
    for iter in ${ITERS}; do
      ...
      header=$(printf '%s[%d/%d]%s seed=%s pct=%s%% iters=%s' "${C_BOLD}" "${run_idx}" "${total_planned}" "${C_RESET}" "${seed}" "${pct}" "${iter}")
      printf '%s\n' "${header}" >&2

      run_start=$(date +%s)
      SEED="${seed}" PERCENTAGE="${pct}" ITERS="${iter}" CORNER="${CORNER}" \
        DEBUG_LEVEL="${DEBUG_LEVEL}" EXPORT_FILES="${EXPORT_FILES}" \
      ...
      wns_b=$(get wns_before_ps  "${result_line}"); wns_a=$(get wns_after_ps   "${result_line}"); wns_d=$(get wns_delta_ps   "${result_line}")
      tns_b=$(get tns_before_ps  "${result_line}"); tns_a=$(get tns_after_ps   "${result_line}"); tns_d=$(get tns_delta_ps   "${result_line}")
      viols_b=$(get viols_before "${result_line}"); viols_a=$(get viols_after  "${result_line}"); viols_d=$(get viols_delta  "${result_line}")
      sa_chg=$(get sa_changed    "${result_line}"); ms=$(get elapsed_ms "${result_line}")
      ...
      printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
        "${seed}" "${pct}" "${iter}" "${wns_b}" "${tns_b}" "${viols_b}" \
        "${wns_r}" "${tns_r}" "${viols_r}" "${wns_rs}" "${tns_rs}" "${viols_rs}" \
        "${wns_a}" "${tns_a}" "${viols_a}" "${wns_d}" "${tns_d}" "${viols_d}" \
      ...
      if [[ "${sa_chg}" == "YES" ]]; then printf '  SA changed netlist: %s%s%s\n' "${C_GREEN}" "${sa_chg}" "${C_RESET}"
      else printf '  SA changed netlist: %s%s%s\n' "${C_DIM}" "${sa_chg}" "${C_RESET}"; fi
...
summary_plain="$(
  echo ""
  echo "============================================= SUMMARY ${DESIGN^^} SA RESULTS ============================================="
  column -t -s $'\t' < "${results_file}"
  awk -F'\t' 'NR>1{wd+=$16;td+=$17;vd+=$18;n+=1;if($19=="YES")ch+=1} END{if(n>0) printf "\nMean over %d runs: WNS delta = %+.4f ps | TNS delta = %+.4f ps | FEP delta = %+.1f | SA changed = %d/%d (%.0f%%)\n",n,wd/n,td/n,vd/n,ch,n,100.0*ch/n}' "${results_file}"
)"
...
head -n1 "${results_file}" | awk -v b="${C_BOLD}" -v r="${C_RESET}" 'BEGIN{FS=OFS="\t"}{for(i=1;i<=NF;i++) $i=b $i r; print}' > "${colored_tsv}"
tail -n +2 "${results_file}" | while IFS=$'\t' read -r f1 f2 f3 f4 f5 f6 f7 f8 f9 f10 f11 f12 f13 f14 f15 f16 f17 f18 f19 f20; do
  cf16=$(fmt_delta "${f16}" wns); cf17=$(fmt_delta "${f17}" tns); cf18=$(fmt_delta "${f18}" fep)
  if [[ "${f19}" == "YES" ]]; then cf19="${C_GREEN}${f19}${C_RESET}"; else cf19="${C_DIM}${f19}${C_RESET}"; fi
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "${f1}" "${f2}" "${f3}" "${f4}" "${f5}" "${f6}" "${f7}" "${f8}" "${f9}" "${f10}" "${f11}" "${f12}" "${f13}" "${f14}" "${f15}" "${cf16}" "${cf17}" "${cf18}" "${cf19}" "${f20}" >> "${colored_tsv}"
done

echo ""
printf '%s%s============================================= SUMMARY %s SA RESULTS =============================================%s\n' "${C_BOLD}" "${C_CYAN}" "${DESIGN^^}" "${C_RESET}"
column -t -s $'\t' < "${colored_tsv}"

awk -F'\t' -v g="${C_GREEN}" -v r="${C_RED}" -v d="${C_DIM}" -v b="${C_BOLD}" -v x="${C_RESET}" '
  ...
  END{if(n>0){wd_avg=wd/n;td_avg=td/n;vd_avg=vd/n
    printf "\n%sMean over %d runs:%s WNS delta = %s%+.4f ps%s | TNS delta = %s%+.4f ps%s | FEP delta = %s%+.1f%s | SA changed = %d/%d (%.0f%%)\n",
    b,n,x,col_wns(wd_avg),wd_avg,x,col_wns(td_avg),td_avg,x,col_fep(vd_avg),vd_avg,x,ch,n,100.0*ch/n}}' "${results_file}"

printf '\n%sSweep elapsed: %ds%s\n' "${C_DIM}" "${sweep_elapsed}" "${C_RESET}"
cp "${results_file}" "${results_tsv}"
echo ""; echo "Raw TSV  : ${results_tsv}"; echo "Summary  : ${results_txt}"; echo "Log dir  : ${LOG_DIR}/"
