...
# post-CTS i2c_master_top design (ASAP7) and aggregate baseline-vs-after WNS/TNS/FEP QoR.
#
# Run from this directory (mcts_script/i2c_master_top/):
#   ./i2c_master_top_bench_routed_qor_mcts.sh
#   SEEDS="50 100" PERCENTAGES="10 20 30" ITERS="100 200" ./i2c_master_top_bench_routed_qor_mcts.sh
#   OPENROAD=../../../../build/bin/openroad ./i2c_master_top_bench_routed_qor_mcts.sh
#   DEBUG_LEVEL=0 ./i2c_master_top_bench_routed_qor_mcts.sh
#   EXPORT_FILES=1 ./i2c_master_top_bench_routed_qor_mcts.sh
#   NO_COLOR=1 ./i2c_master_top_bench_routed_qor_mcts.sh
set -u
...
TCL="${TCL:-i2c_master_top_routed_qor_mcts.tcl}"

DESIGN="${DESIGN:-i2c_master_top}"

# Logs land in ../../qor_mcts/i2c_master_top/ (relative to this script's directory)
...
if [[ ! -f "${TCL}" ]]; then
  echo "ERROR: ${TCL} not found; run from mcts_script/i2c_master_top/" >&2
  exit 1
fi
...
render_progress() {
  ...
  awk -v logfile="${logfile}"       -v total="${total_iters}"       -v width="${bar_width}"       -v header="${header}"       -v cyan="${C_CYAN}" -v dim="${C_DIM}" -v bold="${C_BOLD}" -v reset="${C_RESET}"       -v clr="${CLR_LINE}" '
    BEGIN {
...
for seed in ${SEEDS}; do
  for pct in ${PERCENTAGES}; do
    for iter in ${ITERS}; do
      ...
      run_start=$(date +%s)
      SEED="${seed}" PERCENTAGE="${pct}" ITERS="${iter}" MAX_DEPTH="${MAX_DEPTH}"         UCB_CONSTANT="${UCB_CONSTANT}" CORNER="${CORNER}"         DEBUG_LEVEL="${DEBUG_LEVEL}" EXPORT_FILES="${EXPORT_FILES}"         "${OPENROAD}" -no_init -exit "${TCL}" 2>&1         | render_progress "${log}" "${iter}" "  progress:"
      rc=${PIPESTATUS[0]}
