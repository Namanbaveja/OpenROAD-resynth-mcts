...
# post-PnR spi_final design and aggregate baseline-vs-after WNS/TNS/FEPs QoR.
#
# Live output:
...
#
# Requirements: openroad in PATH, run from sa_script/spi/.
#   ./spi_bench_routed_qor.sh
#   SEEDS="50 100" PERCENTAGES="10 20 30" ITERS="100 200" ./spi_bench_routed_qor.sh
#   OPENROAD=../../../build/bin/openroad ./spi_bench_routed_qor.sh
#   DEBUG_LEVEL=0 ./spi_bench_routed_qor.sh   # suppress per-iter debug noise
#   EXPORT_FILES=1 ./spi_bench_routed_qor.sh  # also write DEF/v/sdc/spef
#   NO_COLOR=1 ./spi_bench_routed_qor.sh      # disable ANSI colors
set -u
...
TCL="${TCL:-spi_routed_qor.tcl}"

# Derive design name from the TCL filename (spi_routed_qor.tcl → "spi").
...
if [[ ! -f "${TCL}" ]]; then
  echo "ERROR: ${TCL} not found; run from sa_script/spi/" >&2
  exit 1
fi
