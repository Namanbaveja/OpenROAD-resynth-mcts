...
# post-PnR i2c_master_top_top_final design and aggregate baseline-vs-after WNS/TNS/FEPs QoR.
#
# Live output:
...
#
# Requirements: openroad in PATH, run from sa_script/i2c_master_top/.
#   ./i2c_master_top_top_bench_routed_qor.sh
#   SEEDS="50 100" PERCENTAGES="10 20 30" ITERS="100 200" ./i2c_master_top_top_bench_routed_qor.sh
#   OPENROAD=../../../build/bin/openroad ./i2c_master_top_top_bench_routed_qor.sh
#   DEBUG_LEVEL=0 ./i2c_master_top_top_bench_routed_qor.sh   # suppress per-iter debug noise
#   EXPORT_FILES=1 ./i2c_master_top_top_bench_routed_qor.sh  # also write DEF/v/sdc/spef
#   NO_COLOR=1 ./i2c_master_top_top_bench_routed_qor.sh      # disable ANSI colors
set -u
...
TCL="${TCL:-i2c_master_top_routed_qor.tcl}"

# Derive design name from the TCL filename (i2c_master_top_routed_qor.tcl → "i2c_master_top_top").
DESIGN="${DESIGN:-$(basename "${TCL}" | sed -E 's/_routed_qor\.tcl$//')}"
...
if [[ ! -f "${TCL}" ]]; then
  echo "ERROR: ${TCL} not found; run from sa_script/i2c_master_top/" >&2
  exit 1
fi
