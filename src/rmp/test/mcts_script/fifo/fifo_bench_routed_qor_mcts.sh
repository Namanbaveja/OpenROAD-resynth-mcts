...
# post-CTS fifo design (ASAP7) and aggregate baseline-vs-after WNS/TNS/FEP QoR.
#
# Run from this directory (mcts_script/fifo/):
#   ./fifo_bench_routed_qor_mcts.sh
#   SEEDS="50 100" PERCENTAGES="10 20 30" ITERS="100 200" ./fifo_bench_routed_qor_mcts.sh
#   OPENROAD=../../../../build/bin/openroad ./fifo_bench_routed_qor_mcts.sh
#   DEBUG_LEVEL=0 ./fifo_bench_routed_qor_mcts.sh
#   EXPORT_FILES=1 ./fifo_bench_routed_qor_mcts.sh
#   NO_COLOR=1 ./fifo_bench_routed_qor_mcts.sh
set -u
...
TCL="${TCL:-fifo_routed_qor_mcts.tcl}"

DESIGN="${DESIGN:-fifo}"

# Logs land in ../../qor_mcts/fifo/ (relative to this script's directory)
...
if [[ ! -f "${TCL}" ]]; then
  echo "ERROR: ${TCL} not found; run from mcts_script/fifo/" >&2
  exit 1
fi
