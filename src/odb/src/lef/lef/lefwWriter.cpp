...
// internal function
bool lefwValidateMaskNumber(int num)
{
  int digit = 0;
  int index = 0;
...
int lefwInit(FILE* f)
{
  ...
  if (lefwHasInitCbk == 1) {  // lefwInitCbk has already called, issue an error
    fprintf(lefwFile,
            "ERROR (LEFWRIT-4000): lefwInitCbk has already called, cannot call "
            "lefwInit again.\n");
...
int lefwInitCbk(FILE* f)
{
  ...
  if (lefwHasInit == 1) {  // lefwInit has already called, issue an error
    if (lefwWriteEncrypt) {
      ...
      fprintf(lefwFile,
              "ERROR (LEFWRIT-4002): lefwInit has already called, cannot call "
              "lefwInitCbk again.\n");
      fprintf(lefwFile, "Writer Exit.\n");
    }
    exit(LEFW_BAD_ORDER);
...
int lefwEncrypt()
{
  if (!lefwFile || (lefwFile == stdout)) {
    ...
    fprintf(stderr,
            "ERROR (LEFWRITE-4006): Need an output file if writing in "
            "encrypted format.\n");
...
)
{
  ...
  if (lefwState != LEFW_UNITS_START && lefwState != LEFW_UNITS) {
    return LEFW_BAD_ORDER;
  }
  ...
  if (capacitance) {
    if (lefwWriteEncrypt) {
      encPrint(
          lefwFile, (char*) "   CAPACITANCE PICOFARADS %.11g ;\n", capacitance);
    } else {
...
int lefwStartLayer(const char* layerName, const char* type)
{
  ...
  if (lefwState == LEFW_LAYER_START || lefwState == LEFW_LAYER) {
    return LEFW_BAD_ORDER;
  }
  if (lefwState != LEFW_INIT && lefwState < LEFW_END) {
    return LEFW_BAD_ORDER;  // not of the ENDs
...
int lefwLayerMask(int colorMask)
{
  if (!lefwFile) {
    return LEFW_UNINITIALIZED;
  }
  ...
  lefwState
      = (lefwState == LEFW_LAYERROUTING_START) ? LEFW_LAYERROUTING : LEFW_LAYER;
  ...
  if (lefwWriteEncrypt) {
    ...
    if (location && (*location != 0)) {
      if (strcmp(location, "ABOVE") && strcmp(location, "BELOW")) {
        encPrint(lefwFile,
                 (char*) "ENCLOSURE in Layer(Cut) can only be ABOVE, BELOW.\n");
        return LEFW_BAD_DATA;
      }
  ...
  if (lefwWriteEncrypt) {
    ...
    if (location && (*location != 0)) {
      if (strcmp(location, "ABOVE") && strcmp(location, "BELOW")) {
        encPrint(
            lefwFile,
            (char*) "PREFERENCLOSURE in Layer(Cut) can only be ABOVE, "
                    "BELOW.\n");
        return LEFW_BAD_DATA;
      }
      encPrint(lefwFile, (char*) "%s ", location);
...
int lefwLayerRouting(const char* direction, double width)
{
  ...
  if (lefwWriteEncrypt) {
    ...
    if (strcmp(direction, "HORIZONTAL") && strcmp(direction, "VERTICAL")
        && strcmp(direction, "DIAG45") && strcmp(direction, "DIAG135")) {
      fprintf(lefwFile,
              "DIRECTION in Layer(Routing) can only be HORIZONTAL, VERTICAL, "
              "DIAG45, DIAG135.\n");
      return LEFW_BAD_DATA;  // direction is either HORIZONTAL, VERTICAL,
                             // DIAG45, DIAG135 only
    }
...
int lefwLayerRoutingDiagPitchXYDistance(double xDistance, double yDistance)
{
  ...
  if (lefwWriteEncrypt) {
    encPrint(
        lefwFile, (char*) "   DIAGPITCH %.11g %.11g ;\n", xDistance, yDistance);
  } else {
                                     ...
                                     double cutDistance)
{
  ...
  if (lefwWriteEncrypt) {
    encPrint(lefwFile,
             (char*) "   MINIMUMCUT %.11g WIDTH %.11g WITHIN %.11g ",
             numCuts,
             minWidth,
             cutDistance);
  } else {
    fprintf(lefwFile,
            "   MINIMUMCUT %.11g WIDTH %.11g WITHIN %.11g ",
            numCuts,
            minWidth,
            cutDistance);
  }
...
int lefwLayerRoutingSpacingRangeUseLengthThreshold()
{
  ...
  if (!lefwHasLayerRoutingRange) {
    ...
    fprintf(lefwFile,
            "ERROR (LEFWRIT-4003): lefwLayerRoutingSpacingUseLengthThreshold "
            "cannot be called if\n");
    fprintf(lefwFile,
                                          ...
                                          double subMaxWidth)
{
  ...
  if (!lefwHasLayerRoutingRange) {
    // This routine can be called only if range are non zero
    fprintf(lefwFile,
            "ERROR (LEFWRIT-4004): lefwLayerRoutingSpacingInfluence cannot be "
            "called if\n");
    fprintf(lefwFile,
            "       lefRange and rightRange in lefwLayerRoutingSpacing are "
            "both zero.\n");
    return LEFW_BAD_DATA;
  }
  if (lefwWriteEncrypt) {
    ...
    if (subMinWidth || subMaxWidth) {
      encPrint(
          lefwFile, (char*) "RANGE %.11g %.11g ", subMinWidth, subMaxWidth);
    }
  } else {
    fprintf(lefwFile, "\n     INFLUENCE %.11g ", infValue);
    if (subMinWidth || subMaxWidth) {
...
int lefwLayerRoutingSpacingRangeRange(double minWidth, double maxWidth)
{
  ...
  if (!lefwHasLayerRoutingRange) {
    // This routine can be called only if range are non zero
    fprintf(lefwFile,
            "ERROR (LEFWRIT-4005): lefwLayerRoutingSpacingInfluence cannot be "
            "called if\n");
    fprintf(lefwFile,
            "       lefRange and rightRange in lefwLayerRoutingSpacing are "
            "both zero.\n");
                                       ...
                                       int twoEdges)
{
  ...
  if (lefwWriteEncrypt) {
    if (twoEdges) {
      encPrint(lefwFile,
               (char*) "PARALLELEDGE %.11g WITHIN %.11g TWOEDGES ;\n",
               parSpace,
               parWithin);
    } else {
      encPrint(lefwFile,
               (char*) "PARALLELEDGE %.11g WITHIN %.11g ;\n",
               parSpace,
               parWithin);
    }
  } else {
    if (twoEdges) {
      fprintf(lefwFile,
              "PARALLELEDGE %.11g WITHIN %.11g TWOEDGES ;\n",
              parSpace,
              parWithin);
    } else {
      fprintf(
          lefwFile, "PARALLELEDGE %.11g WITHIN %.11g ;\n", parSpace, parWithin);
    }
  }
  lefwLines++;
                                           ...
                                           double minNLength)
{
  ...
  if (lefwWriteEncrypt) {
    ...
    fprintf(
        lefwFile,
        "    ENDOFNOTCHWIDTH %.11g NOTCHSPACING %.11g NOTCHLENGTH %.11g ;\n",
        eonWidth,
        minNSpacing,
        minNLength);
  }
...
int lefwLayerRoutingAntennaLength(double antennaLength)
{
  ...
  if (lefwWriteEncrypt) {
    encPrint(
        lefwFile, (char*) "   ANTENNALENGTHFACTOR %.11g ;\n", antennaLength);
  } else {
                                       ...
                                       double maxLength)
{
  ...
  if (rule
      && ((strcmp(rule, "INSIDECORNER") == 0)
          || (strcmp(rule, "OUTSIDECORNER") == 0)
          || (strcmp(rule, "STEP") == 0))) {
    if (lefwWriteEncrypt) {
...
int lefwLayerRoutingMinstepMaxEdges(double distance, double maxEdges)
{
  ...
  if (lefwWriteEncrypt) {
    encPrint(lefwFile,
             (char*) "   MINSTEP %.11g MAXEDGES %.11g ;\n",
             distance,
             maxEdges);
  } else {
    fprintf(
        lefwFile, "   MINSTEP %.11g MAXEDGES %.11g ;\n", distance, maxEdges);
  }
  lefwLines++;
  return LEFW_OK;
}
...
int lefwLayerRoutingProtrusion(double width1, double length, double width2)
{
  ...
  if (lefwWriteEncrypt) {
    encPrint(lefwFile,
             (char*) "   PROTRUSIONWIDTH %.11g LENGTH %.11g WIDTH %.11g ;\n",
             width1,
             length,
             width2);
  } else {
...
int lefwLayerAntennaCumDiffSideAreaRatio(double value)
{
  ...
  if (lefwWriteEncrypt) {
    encPrint(
        lefwFile, (char*) "   ANTENNACUMDIFFSIDEAREARATIO %.11g ;\n", value);
  } else {
...
int lefwLayerAntennaGatePlusDiff(double plusDiffFactor)
{
  ...
  */
  if (lefwWriteEncrypt) {
    encPrint(
        lefwFile, (char*) "   ANTENNAGATEPLUSDIFF %.11g ;\n", plusDiffFactor);
  } else {
...
int lefwLayerAntennaAreaMinusDiff(double minusDiffFactor)
{
  ...
  */
  if (lefwWriteEncrypt) {
    encPrint(
        lefwFile, (char*) "   ANTENNAAREAMINUSDIFF %.11g ;\n", minusDiffFactor);
  } else {
                                      ...
                                      double* metalDiffFactors)
{
  ...
  */
  ...
  if (lefwWriteEncrypt) {
    ...
    for (i = 0; i < numPwls; i++) {
      fprintf(lefwFile, "( %.11g %.11g ) ", *diffAreas++, *metalDiffFactors++);
    }
    fprintf(lefwFile, ") ;\n");
  }
  lefwLines++;
...
int lefwDensityCheckWindow(double checkWindowLength, double checkWindowWidth)
{
  ...
  if (lefwWriteEncrypt) {
    encPrint(lefwFile,
             (char*) "   DENSITYCHECKWINDOW %.11g %.11g ;\n",
             checkWindowLength,
             checkWindowWidth);
  } else {
    fprintf(lefwFile,
            "   DENSITYCHECKWINDOW %.11g %.11g ;\n",
            checkWindowLength,
            checkWindowWidth);
  }
  lefwLines++;
                   ...
                   double yTopEnc)
{
  ...
  if (lefwWriteEncrypt) {
    ...
    encPrint(lefwFile,
             ...
             topMetalLayer);
    encPrint(lefwFile,
             (char*) "     CUTSPACING %.11g %.11g ;\n",
             xCutSpacing,
             yCutSpacing);
    encPrint(lefwFile,
             (char*) "     ENCLOSURE %.11g %.11g %.11g %.11g ;\n",
             xBotEnc,
             yBotEnc,
             xTopEnc,
             yTopEnc);
...
// internal function
int lefwViaRulePrtLayer(const char* layerName,
                        ...
                        double metalOverhang)
{
  ...
  if (lefwWriteEncrypt) {
    ...
    if (metalOverhang) {
      encPrint(
          lefwFile, (char*) "      METALOVERHANG %.11g ;\n", metalOverhang);
      lefwLines++;
    }
  } else {
    if (direction) {
...
int lefwStartViaRule(const char* viaRuleName)
{
  ...
  if (lefwState == LEFW_VIARULE_START || lefwState == LEFW_VIARULE) {
    return LEFW_BAD_ORDER;
  }
  if (lefwState != LEFW_INIT && lefwState < LEFW_END) {
    return LEFW_BAD_ORDER;  // not of the ENDs
...
int lefwStartViaRuleGen(const char* viaRuleName)
{
  ...
  if (lefwState == LEFW_VIARULEGEN_START || lefwState == LEFW_VIARULEGEN) {
    return LEFW_BAD_ORDER;
  }
  if (lefwState != LEFW_INIT && lefwState < LEFW_END) {
    return LEFW_BAD_ORDER;  // not of the ENDs
                            ...
                            double edgeCap)
{
  ...
  if (lefwWriteEncrypt) {
    ...
    if (wireExtension) {
      encPrint(
          lefwFile, (char*) "      WIREEXTENSION %.11g ;\n", wireExtension);
      lefwLines++;
    }
    if (resist) {
    ...
    if (capacitance) {
      encPrint(lefwFile,
               (char*) "      CAPACITANCE CPERSQDIST %.11g ;\n",
               capacitance);
      lefwLines++;
    }
    if (edgeCap) {
...
int lefwEndNonDefaultRule(const char* ruleName)
{
  ...
  if (lefwState != LEFW_NONDEFAULTRULE_START && lefwState != LEFW_NONDEFAULTRULE
...
{  // optioanl
  ...
  if (lefwWriteEncrypt) {
    ...
    fprintf(
        lefwFile, "   SAMENET %s %s %.11g ", layerName1, layerName2, minSpace);
    if (stack && strcmp(stack, "STACK") == 0) {
      fprintf(lefwFile, "STACK ");
    }
    fprintf(lefwFile, ";\n");
...
int lefwUniversalNoiseMargin(double high, double low)
{
  ...
  if (lefwWriteEncrypt) {
    encPrint(
        lefwFile, (char*) "UNIVERSALNOISEMARGIN %.11g %.11g ;\n", high, low);
  } else {
...
int lefwStartNoiseTable(int num)
{
  ...
  if (lefwState == LEFW_NOISETABLE_START || lefwState == LEFW_NOISETABLE) {
    return LEFW_BAD_ORDER;
  }
  if (lefwState != LEFW_INIT && lefwState < LEFW_END) {
    return LEFW_BAD_ORDER;  // not of the ENDs
...
int lefwStartCorrectTable(int num)
{
  ...
  if (lefwState == LEFW_CORRECTTABLE_START || lefwState == LEFW_CORRECTTABLE) {
    return LEFW_BAD_ORDER;
  }
  if (lefwState != LEFW_INIT && lefwState < LEFW_END) {
    return LEFW_BAD_ORDER;  // not of the ENDs
                          ...
                          double spaceY)
{
  ...
  if (lefwWriteEncrypt) {
    ...
    fprintf(lefwFile,
            "   CANNOTOCCUPY %s %.11g %.11g %s DO %.11g BY %.11g STEP %.11g "
            "%.11g ;\n",
            name,
            origX,
            origY,
                    ...
                    const char* layers)
{
  ...
  if (lefwWriteEncrypt) {
    ...
    fprintf(lefwFile,
            "   TRACKS %s %.11g DO %d STEP %.11g LAYER %s ;\n",
            xy,
            start,
            numTracks,
            space,
                       ...
                       double spaceXY)
{
  ...
  if (lefwWriteEncrypt) {
    encPrint(lefwFile,
             (char*) "   GCELLGRID %s %.11g DO %d STEP %.11g ;\n",
             xy,
             startXY,
             colRows,
             spaceXY);
...
int lefwMacroPinAntennaPartialMetalSideArea(double value, const char* layerName)
{
  ...
  if (lefwWriteEncrypt) {
    encPrint(
        lefwFile, (char*) "      ANTENNAPARTIALMETALSIDEAREA %.11g ", value);
    if (layerName) {
      encPrint(lefwFile, (char*) "LAYER %s ", layerName);
    }
    encPrint(lefwFile, (char*) ";\n");
...
{  // optional
  ...
  if (lefwWriteEncrypt) {
    if (numX || numY || spaceX || spaceY) {
      if (mask) {
        encPrint(
            lefwFile, (char*) "            POLYGON MASK %d ITERATE ", mask);
      } else {
...
{  // optional
  ...
  if (lefwWriteEncrypt) {
    if (numX || numY || spaceX || spaceY) {
      if (mask) {
        encPrint(lefwFile,
                 (char*) "            VIA ITERATE MASK %d %.11g %.11g %s ",
                 mask,
                 xl,
                 yl,
                 viaName);
      ...
      encPrint(
          lefwFile, (char*) "            VIA %.11g %.11g %s ", xl, yl, viaName);
    }
    encPrint(lefwFile, (char*) ";\n");
  } else {
    if (numX || numY || spaceX || spaceY) {
                             ...
                             double varMax)
{
  ...
  if (lefwWriteEncrypt) {
    encPrint(lefwFile,
             (char*) "      %s INTRINSIC %.11g %.11g ",
             riseFall,
             min,
             max);
    if (slewT1 || slewT1Min || slewT1Max || slewT2) {
      encPrint(lefwFile,
               (char*) "%.11g %.11g %.11g %.11g ",
               slewT1,
               slewT1Min,
               slewT1Max,
               slewT2);
    ...
    encPrint(
        lefwFile, (char*) "         VARIABLE %.11g %.11g ;\n", varMin, varMax);
  } else {
...
int lefwAntennaOutputDiffArea(double outputDiffArea)
{
  ...
  if (lefwWriteEncrypt) {
    encPrint(
        lefwFile, (char*) "ANTENNAOUTPUTDIFFAREA %.11g ;\n", outputDiffArea);
  } else {
  ...
  switch (status) {
    ...
    case LEFW_WRONG_VERSION:
      if (lefwWriteEncrypt) {
        ...
        fprintf(
            lefwFile,
            "Version number is set before 5.4, but 5.4, API: %s is for 5.4.\n",
            lefwStateStr[lefw54Num]);
      }
      break;
    case LEFW_MIX_VERSION_DATA:
      if (lefwWriteEncrypt) {
        ...
        fprintf(lefwFile,
                "You have made both 5.4 & pre 5.4 Antenna API called, which is "
                "invalid.\n");
      }
      break;
    case LEFW_OBSOLETE:
      if (lefwWriteEncrypt) {
        encPrint(lefwFile,
                 (char*) "%s is no longer valid in %g.\n",
