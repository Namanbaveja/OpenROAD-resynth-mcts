...
# post-CTS sasc_top design (ASAP7) and aggregate baseline-vs-after WNS/TNS/FEP QoR.
#
# Run from this directory (mcts_script/sasc_top/):
#   ./sasc_top_bench_routed_qor_mcts.sh
#   SEEDS="50 100" PERCENTAGES="10 20 30" ITERS="100 200" ./sasc_top_bench_routed_qor_mcts.sh
#   OPENROAD=../../../../build/bin/openroad ./sasc_top_bench_routed_qor_mcts.sh
#   DEBUG_LEVEL=0 ./sasc_top_bench_routed_qor_mcts.sh
#   EXPORT_FILES=1 ./sasc_top_bench_routed_qor_mcts.sh
#   NO_COLOR=1 ./sasc_top_bench_routed_qor_mcts.sh
set -u
...
TCL="${TCL:-sasc_top_routed_qor_mcts.tcl}"

DESIGN="${DESIGN:-sasc_top}"

# Logs land in ../../qor_mcts/sasc_top/ (relative to this script's directory)
...
if [[ ! -f "${TCL}" ]]; then
  echo "ERROR: ${TCL} not found; run from mcts_script/sasc_top/" >&2
  exit 1
fi
