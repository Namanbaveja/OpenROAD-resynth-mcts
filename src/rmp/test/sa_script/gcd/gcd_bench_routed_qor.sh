...
# post-PnR gcd_final design and aggregate baseline-vs-after WNS/TNS/FEPs QoR.
#
# Live output:
...
#
# Requirements: openroad in PATH, run from sa_script/gcd/.
#   ./gcd_bench_routed_qor.sh
#   SEEDS="50 100" PERCENTAGES="10 20 30" ITERS="100 200" ./gcd_bench_routed_qor.sh
#   OPENROAD=../../../build/bin/openroad ./gcd_bench_routed_qor.sh
#   DEBUG_LEVEL=0 ./gcd_bench_routed_qor.sh   # suppress per-iter debug noise
#   EXPORT_FILES=1 ./gcd_bench_routed_qor.sh  # also write DEF/v/sdc/spef
#   NO_COLOR=1 ./gcd_bench_routed_qor.sh      # disable ANSI colors
set -u
...
TCL="${TCL:-gcd_routed_qor.tcl}"

# Derive design name from the TCL filename (gcd_routed_qor.tcl → "gcd").
...
if [[ ! -f "${TCL}" ]]; then
  echo "ERROR: ${TCL} not found; run from sa_script/gcd/" >&2
  exit 1
fi
