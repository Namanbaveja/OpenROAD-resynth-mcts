...
# post-CTS mem_ctrl design (ASAP7) and aggregate baseline-vs-after WNS/TNS/FEP QoR.
#
# Run from this directory (mcts_script/mem_ctrl/):
#   ./mem_ctrl_bench_routed_qor_mcts.sh
#   SEEDS="50 100" PERCENTAGES="10 20 30" ITERS="100 200" ./mem_ctrl_bench_routed_qor_mcts.sh
#   OPENROAD=../../../../build/bin/openroad ./mem_ctrl_bench_routed_qor_mcts.sh
#   DEBUG_LEVEL=0 ./mem_ctrl_bench_routed_qor_mcts.sh
#   EXPORT_FILES=1 ./mem_ctrl_bench_routed_qor_mcts.sh
#   NO_COLOR=1 ./mem_ctrl_bench_routed_qor_mcts.sh
set -u
...
TCL="${TCL:-mem_ctrl_routed_qor_mcts.tcl}"

DESIGN="${DESIGN:-mem_ctrl}"

# Logs land in ../../qor_mcts/mem_ctrl/ (relative to this script's directory)
...
if [[ ! -f "${TCL}" ]]; then
  echo "ERROR: ${TCL} not found; run from mcts_script/mem_ctrl/" >&2
  exit 1
fi
