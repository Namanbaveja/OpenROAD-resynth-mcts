...
proc changeAndTest { obj SetterName GetterName expectedVal args } {
  ...
  if { [string is ascii $expectedVal] } {
    assertStringEq [$obj $GetterName] $expectedVal \
      [format "%s/%s failed: %s %s" $SetterName $GetterName [$obj $GetterName] $expectedVal]
  } else {
    assert {[$obj $GetterName] == $expectedVal} \
      [format "%s/%s failed: %g %g" $SetterName $GetterName [$obj $GetterName] $expectedVal]
  }
}
