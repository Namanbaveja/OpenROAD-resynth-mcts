...
# post-CTS wb_dma design (ASAP7) and aggregate baseline-vs-after WNS/TNS/FEP QoR.
#
# Run from this directory (mcts_script/wb_dma/):
#   ./wb_dma_bench_routed_qor_mcts.sh
#   SEEDS="50 100" PERCENTAGES="10 20 30" ITERS="100 200" ./wb_dma_bench_routed_qor_mcts.sh
#   OPENROAD=../../../../build/bin/openroad ./wb_dma_bench_routed_qor_mcts.sh
#   DEBUG_LEVEL=0 ./wb_dma_bench_routed_qor_mcts.sh
#   EXPORT_FILES=1 ./wb_dma_bench_routed_qor_mcts.sh
#   NO_COLOR=1 ./wb_dma_bench_routed_qor_mcts.sh
set -u
...
TCL="${TCL:-wb_dma_routed_qor_mcts.tcl}"

DESIGN="${DESIGN:-wb_dma}"

# Logs land in ../../qor_mcts/wb_dma/ (relative to this script's directory)
...
if [[ ! -f "${TCL}" ]]; then
  echo "ERROR: ${TCL} not found; run from mcts_script/wb_dma/" >&2
  exit 1
fi
