...
# post-CTS vga_led design (ASAP7) and aggregate baseline-vs-after WNS/TNS/FEP QoR.
#
# Run from this directory (mcts_script/vga_led/):
#   ./vga_led_bench_routed_qor_mcts.sh
#   SEEDS="50 100" PERCENTAGES="10 20 30" ITERS="100 200" ./vga_led_bench_routed_qor_mcts.sh
#   OPENROAD=../../../../build/bin/openroad ./vga_led_bench_routed_qor_mcts.sh
#   DEBUG_LEVEL=0 ./vga_led_bench_routed_qor_mcts.sh
#   EXPORT_FILES=1 ./vga_led_bench_routed_qor_mcts.sh
#   NO_COLOR=1 ./vga_led_bench_routed_qor_mcts.sh
set -u
...
TCL="${TCL:-vga_led_routed_qor_mcts.tcl}"

DESIGN="${DESIGN:-vga_led}"

# Logs land in ../../qor_mcts/vga_led/ (relative to this script's directory)
...
if [[ ! -f "${TCL}" ]]; then
  echo "ERROR: ${TCL} not found; run from mcts_script/vga_led/" >&2
  exit 1
fi
