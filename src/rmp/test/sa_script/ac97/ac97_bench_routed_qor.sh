...
# post-PnR ac97_final design and aggregate baseline-vs-after WNS/TNS/FEPs QoR.
#
# Live output:
...
#
# Requirements: openroad in PATH, run from sa_script/ac97/.
#   ./ac97_bench_routed_qor.sh
#   SEEDS="50 100" PERCENTAGES="10 20 30" ITERS="100 200" ./ac97_bench_routed_qor.sh
#   OPENROAD=../../../build/bin/openroad ./ac97_bench_routed_qor.sh
#   DEBUG_LEVEL=0 ./ac97_bench_routed_qor.sh   # suppress per-iter debug noise
#   EXPORT_FILES=1 ./ac97_bench_routed_qor.sh  # also write DEF/v/sdc/spef
#   NO_COLOR=1 ./ac97_bench_routed_qor.sh      # disable ANSI colors
set -u
...
TCL="${TCL:-ac97_routed_qor.tcl}"

# Derive design name from the TCL filename (ac97_routed_qor.tcl → "ac97").
...
if [[ ! -f "${TCL}" ]]; then
  echo "ERROR: ${TCL} not found; run from sa_script/ac97/" >&2
  exit 1
fi
