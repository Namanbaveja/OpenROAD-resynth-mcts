#!/usr/bin/env bash
# Sweep resynth_mcts on post-CTS simple_spi_top (ASAP7).
# Run from mcts_script/simple_spi_top/:
#   ./simple_spi_top_bench_routed_qor_mcts.sh
#   SEEDS="50 100" PERCENTAGES="10 20 30" ITERS="500" ./simple_spi_top_bench_routed_qor_mcts.sh
#   OPENROAD=../../../../build/bin/openroad ./simple_spi_top_bench_routed_qor_mcts.sh
set -u
...
TCL="${TCL:-simple_spi_top_routed_qor_mcts.tcl}"
...
if [[ ! -f "${TCL}" ]]; then
  echo "ERROR: ${TCL} not found; run from mcts_script/simple_spi_top/" >&2
  exit 1
fi
...
for seed in ${SEEDS}; do
  for pct in ${PERCENTAGES}; do
    for iter in ${ITERS}; do
      ...
      mcts_chg=$(get mcts_changed "${result_line}"); ms=$(get elapsed_ms "${result_line}")
      ...
      if [[ "${mcts_chg}" == "YES" ]]; then printf '  MCTS changed netlist: %s%s%s\n' "${C_GREEN}" "${mcts_chg}" "${C_RESET}"
      else printf '  MCTS changed netlist: %s%s%s\n' "${C_DIM}" "${mcts_chg}" "${C_RESET}"; fi
