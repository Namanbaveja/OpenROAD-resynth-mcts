...
# post-PnR wb_dma_final design and aggregate baseline-vs-after WNS/TNS/FEPs QoR.
#
# Live output:
...
#
# Requirements: openroad in PATH, run from sa_script/wb_dma/.
#   ./wb_dma_bench_routed_qor.sh
#   SEEDS="50 100" PERCENTAGES="10 20 30" ITERS="100 200" ./wb_dma_bench_routed_qor.sh
#   OPENROAD=../../../build/bin/openroad ./wb_dma_bench_routed_qor.sh
#   DEBUG_LEVEL=0 ./wb_dma_bench_routed_qor.sh   # suppress per-iter debug noise
#   EXPORT_FILES=1 ./wb_dma_bench_routed_qor.sh  # also write DEF/v/sdc/spef
#   NO_COLOR=1 ./wb_dma_bench_routed_qor.sh      # disable ANSI colors
set -u
...
TCL="${TCL:-wb_dma_routed_qor.tcl}"

# Derive design name from the TCL filename (wb_dma_routed_qor.tcl → "wb_dma").
...
if [[ ! -f "${TCL}" ]]; then
  echo "ERROR: ${TCL} not found; run from sa_script/wb_dma/" >&2
  exit 1
fi
